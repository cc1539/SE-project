...
if($_SERVER['REQUEST_METHOD']=="POST") {
	...
	if($_POST['activity']=="enter") {
		...
		$sqlquery = "UPDATE accounttable SET LatestEnterTime=now() WHERE AccountID='".$accountid."';";
		$sqlresult = $SQL->query($sqlquery);
		if(!$sqlresult) {
		...
		$sqlquery = "UPDATE accounttable SET PayAmount=PayAmount+TimeDiff(now(),LatestEnterTime) WHERE AccountID=".$accountid.";";
		$sqlresult = $SQL->query($sqlquery);
		if(!$sqlresult) {
			die("time_log_failure");
		}
