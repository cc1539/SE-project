...
function buildLoginForm(response) {
	...
	if($(window).width()<800) {
		$("#login-div .toggle").show();
		$("#mid-split").hide();
		$("#signup").css("width","calc(100% - 50px)");
		$("#signin").css("width","calc(100% - 50px)");
		$("#signup").hide();
...
function updateFormatting() {
	...
	$("#header").attr("style","");
	$("#header > #top-color").attr("style","");
	$("#header > .group").attr("style","");
	$("#header > .links").attr("style","");
	$("#header > .links > .link").attr("style","");
	$("#virtual-map").attr("style","");
	$("#user-features").attr("style","");
	if(width<750) {
		$("#header #logo").css("font-size","24px");
		$("#header #logo").css("left","10px");
		$("#header #logo").css("top","20px");
		$("#header .subtext").hide();
		$("#header").css("height","150px");
		$("#header > #top-color").css("height","130px");
		$("#header > .group").css("position","relative");
		$("#header > .group").css("left","0");
		$("#header > .links").css("left","calc(230px - 50%)");
		$("#header > .links > .link").css("padding-left","10px");
		$("#user-features").css("padding-left","0px");
		$("#user-features").css("padding-right","0px");
		$("#user-features").css("margin-left","0px");
		$("#user-features").css("margin-right","0px");
		$("#user-features").css("width","100%");
		$("#user-features #virtual-map").css("margin","0px");
		$("#user-features #virtual-map").css("padding","0px");
	} else if(width<1110) {
		$("#header .subtext").show();
		$("#header").css("height","150px");
		$("#header > #top-color").css("height","130px");
		$("#header > .group").css("position","relative");
		$("#header > .group").css("left","calc(50% - 350px)");
		$("#header > .links").css("left","calc(230px - 50%)");
	}
	
	$("#account-info").appendTo($("#user-features > .pair-container"));
	$("#account-info").attr("style","");
	if(width<1010) {
		$("#account-info").appendTo($("#user-features"));
		$("#account-info").css("padding-bottom","100px");
	}
...
$(function(){
	...
	$(window).resize(updateFormatting);
	updateFormatting();
